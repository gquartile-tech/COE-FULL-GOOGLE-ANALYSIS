...
Writes Google Mastery pillar results to CoE_Account_Mastery_Analysis_Templates.xlsm.
Same template as Amazon Mastery — the reference tab structure is identical.
Writes: D = STATUS, H = What We Saw, I = Why It Matters
"""
...
from config_google_mastery import SCORING_EXCLUDED, PRIORITY_POINTS, IMPORTANCE, MAX_FINDINGS
...
def write_mastery_output(template_path: str, output_path: str, results: dict, ctx: GoogleContext) -> None:
    ...
    _safe_write(ws_main, "A1", f"{ctx.hash_name} — Google Account Mastery Analysis")
    ...
    for cid, res in results.items():
        if cid not in cid_to_row:
            print(f"[writer_mastery] WARNING: {cid} not in reference tab — skipping.")
            continue
        rr = cid_to_row[cid]
    ...
    wb.save(output_path)
    print(f"[writer_mastery] Saved: {output_path} | Score: {score} | Grade: {grade}")
